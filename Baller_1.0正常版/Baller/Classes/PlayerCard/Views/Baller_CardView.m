...
- (id)initWithFrame:(CGRect)frame
    ...
    if (self) {
        if (ballerCardType <= kBallerCardType_MyPlayerCard ) {
            ...
            self.contentSize =CGSizeMake(frame.size.width, NUMBER(750.0, 680.0, 610, 610));
            [self pcv_SetupBackLayerWith:CGRectMake(0.0, 0.0, frame.size.width,  NUMBER(615, 550, 485,485))];
        ...
        switch (ballerCardType) {
            ...
            case kBallerCardType_CreateBasketBallTeam:
            {
                ...
                Baller_CreateBasketBallTeamView * createTeamView = [[[NSBundle mainBundle]loadNibNamed:@"Baller_CreateBasketBallTeamView" owner:nil options:nil] firstObject];
                createTeamView.frame = CGRectMake(0.0, CGRectGetMaxY(_nickNameLabel.frame), self.frame.size.width, 285);
                [self.bottomButton setTitle:@"创建球队" forState:UIControlStateNormal];
                [self addSubview:createTeamView];
...
- (void)setAbilityDetails:(NSArray *)abilityDetails{
    ...
    abilityEditorView.detailButtonClicked = ^(){
...
- (void)addBottomSegments{
    ...
    NSArray * parameters = @[$str(@"%@",[USER_DEFAULT valueForKey:Baller_UserInfo_Height]),$str(@"%@",[USER_DEFAULT valueForKey:Baller_UserInfo_Weight]),$str(@"%@",[USER_DEFAULT valueForKey:Baller_UserInfo_Position])];
