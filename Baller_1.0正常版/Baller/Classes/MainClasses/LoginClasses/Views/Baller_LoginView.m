...
- (void)getSMSCAPTCHA{
    DLog(@"%s",__FUNCTION__);
    __WEAKOBJ(weakSelf, self);
    [AFNHttpRequestOPManager getWithSubUrl:Baller_get_code parameters:@{@"mobile":_userNameTF.text} responseBlock:^(id result, NSError *error) {
        if (error) {
...
- (void)loginButtonAction:(FlatButton *)button{
    ...
    //判断输入的内容
    if (_userNameTF.text.length && _passwordNameTF.text.length) {
...
- (void)userLogin
{
    [AFNHttpRequestOPManager postWithSubUrl:Baller_login parameters:@{@"type":@"normal",@"mobile":_userNameTF.text,@"password":_passwordNameTF.text,@"devicetoken":@""} responseBlock:^(id result, NSError *error) {
