...
- (UIButton *)headImageButton
{
    if (!_headImageButton) {
        ...
        [button setTitle:NSLocalizedString(@"SubmitHeadImage", nil) forState:UIControlStateNormal];
        ...
        //加载头像
        UIImage * plachoderImage = nil;
        if ([USER_DEFAULT valueForKey:Baller_UserInfo_HeadImageData]) {
            plachoderImage = [UIImage imageWithData:[USER_DEFAULT valueForKey:Baller_UserInfo_HeadImageData]];
        }else{
            plachoderImage = [UIImage imageNamed:@"ballPark_default"];
        }
        
        [button setImageForState:UIControlStateNormal withURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[USER_DEFAULT valueForKey:Baller_UserInfo_HeadImage]]] placeholderImage:plachoderImage success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
...
//    [personInfoDic setValue:[[USER_DEFAULT valueForKey:Baller_UserInfo] valueForKey:@"password"] forKey:@"password"];
    ...
    for (int i = 0; i < _infoItemViews.count; i++) {
        ...
        switch (infoItemView.infoTextField.tag) {
            case 1000:
                ...
                [personInfoDic setValue:_nikeNameTextField.text forKey:@"height"];
