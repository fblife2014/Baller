...
- (Baller_PersonalInfoView *)personalInfoView
{
    if (!_personalInfoView) {
        ...
        NSArray * infoDetails;
        if ([USER_DEFAULT valueForKey:Baller_UserInfo_Height]) {
        
            infoDetails = @[[USER_DEFAULT valueForKey:Baller_UserInfo_Height],[USER_DEFAULT valueForKey:Baller_UserInfo_Weight],[USER_DEFAULT valueForKey:Baller_UserInfo_Position],[USER_DEFAULT valueForKey:Baller_UserInfo_Sex]];
        }

        __WEAKOBJ(weakSelf, self);

        [_personalInfoView addPersonInfoViewWithTitles:titles placeHolders:placeHolders infoDetails:infoDetails canEdited:YES originY:(personalInfoView.frame.size.height-5*PersonInfoCell_Height) circleRadius:3.5];
