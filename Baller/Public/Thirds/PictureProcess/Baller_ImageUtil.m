...
@implementation Baller_ImageUtil
...
+ (UIImage *)imageWithImage:(UIImage*)inImage withColorMatrix:(const float*)f scaledToSize:(CGSize)newSize{

    UIGraphicsBeginImageContext(newSize);//根据当前大小创建一个基于位图图形的环境
    
    [inImage drawInRect:CGRectMake(0,0,newSize.width,newSize.height)];//根据新的尺寸画出传过来的图片
    
    UIImage *newImage = UIGraphicsGetImageFromCurrentImageContext();//从当前环境当中得到重绘的图片
    
    UIGraphicsEndImageContext();//关闭当前环境
    
    return [self imageWithImage:newImage withColorMatrix:f];
}
